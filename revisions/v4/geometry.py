...
"""Bathroom window-sill draining board — v4 geometry. Every parameter lives here.

v4 changes no geometry. It changes the print settings carried in the 3MF
...
v3's 4 walls and 5 mm brim were silently reset to stock the moment the
project was opened in the app. Everything below is v2 unchanged.
...
# ---- underside runners: the "feet" ----
# Channels run front-to-back, across the tile ridges, so air reaches under
# the whole board and anything that gets under it can dry. The roof is a
# 45 degree gable (depth = half the width), so nothing bridges.
chan_w, chan_d = 6.0, 3.0              # channel width at the sill, apex height
chan_pitch = 9.6                       # leaves 3.6 runners (0.4 multiple)
chan_n = 16                            # per part, centred, clear of the seam pockets
chan_y1 = sill_d + 5.0                 # vent just past the wall face, short of the drip groove
...
def channel_xs():
    """Centres of the underside channels, the same pattern in every part."""
    out = []
    for x0, x1 in zip(part_x[:-1], part_x[1:]):
        c = (x0 + x1) / 2
        out += [c + (i - (chan_n - 1)/2) * chan_pitch for i in range(chan_n)]
    return out
...
def channels():
    """Gable-roofed channels, open at the back face, venting under the overhang."""
    out = []
    for x in channel_xs():
        prof = Polygon([(x - chan_w/2, -0.5), (x + chan_w/2, -0.5), (x + chan_w/2, 0.0),
                        (x, chan_d), (x - chan_w/2, 0.0)])
        m = trimesh.creation.extrude_polygon(prof, chan_y1 + 1)
        m.apply_transform(trimesh.transformations.rotation_matrix(np.pi/2, [1, 0, 0]))
        m.apply_translation([0, chan_y1, 0])
        out.append(m)
    return out

def underside_cuts():
    return [drip_groove(), *channels()]
