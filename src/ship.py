...
build.py names its output with the tag in it (windowsill-part1-v2.stl). The
...
ALIASES = [
    ('windowsill-%s-views.png',     'views.png'),
    ('windowsill-%s-schematic.png', 'schematic.png'),
    ('windowsill-%s.3mf',           'windowsill.3mf'),
] + [('windowsill-part%d-%%s.stl' % i, 'windowsill-part%d.stl' % i) for i in range(1, 6)]
...
def main(argv):
    ...
    geom = os.path.join(ROOT, 'src', 'geom%02d.py' % int(tag[1:]))
